...
#ifndef MFP
	/* NOTE(mainline): the MFP block above already advertises these two.
	 * Listing them again makes them duplicate cipher suites, which mainline
	 * wiphy_register()->wiphy_cipher_suites_valid() rejects with -EINVAL
	 * (no WARN). Only advertise here when MFP is disabled.
	 */
	WLAN_CIPHER_SUITE_BIP_GMAC_128,
	WLAN_CIPHER_SUITE_BIP_GMAC_256,
#endif /* !MFP */
...
static s32 wl_setup_wiphy(struct wireless_dev *wdev, struct device *sdiofunc_dev, void *context)
{
	...
	if (unlikely(!wdev->wiphy)) {
		...
		pr_err("BCMDHD_DIAG: wiphy_new FAILED\n");
		err = -ENOMEM;
		return err;
	}
	...
	*/
	...
	if (unlikely(err < 0)) {
		...
		pr_err("BCMDHD_DIAG: wiphy_register FAILED err=%d\n", err);
...
s32 wl_cfg80211_attach(struct net_device *ndev, void *context)
{
	...
	if (unlikely(err)) {
		pr_err("BCMDHD_DIAG: attach: wl_setup_wiphy FAILED err=%d\n", err);
	...
	if (err) {
		...
		pr_err("BCMDHD_DIAG: attach: alloc_netinfo FAILED err=%d\n", err);
		goto cfg80211_attach_out;
	}
	err = wl_init_priv(cfg);
	if (err) {
		WL_ERR(("Failed to init iwm_priv (%d)\n", err));
		pr_err("BCMDHD_DIAG: attach: init_priv FAILED err=%d\n", err);
		goto cfg80211_attach_out;
	}
	...
	if (err) {
		...
		pr_err("BCMDHD_DIAG: attach: setup_rfkill FAILED err=%d\n", err);
		goto cfg80211_attach_out;
	}
	...
	if (!wl_cfg80211_netdev_notifier_registered) {
		...
		pr_err("BCMDHD_DIAG: attach: netdev_notifier FAILED err=%d\n", err);
			goto cfg80211_attach_out;
		}
	}
	...
	if (cfg->chan_info_list == NULL) {
		...
		pr_err("BCMDHD_DIAG: attach: chan_info_list FAILED err=%d\n", err);
		goto cfg80211_attach_out;
	}
	...
	if (!cfg->btcoex_info) {
		pr_err("BCMDHD_DIAG: attach: btcoex_init FAILED\n");
		goto cfg80211_attach_out;
	}
#endif /* defined(OEM_ANDROID) */
	...
	if (err) {
		pr_err("BCMDHD_DIAG: attach: cfgp2p_attach_p2p FAILED err=%d\n", err);
		goto cfg80211_attach_out;
	}
	...
	if (err) {
		...
		pr_err("BCMDHD_DIAG: attach: nan_attach FAILED err=%d\n", err);
		goto cfg80211_attach_out;
	}
#endif /* WL_NAN */
	...
	if (err != BCME_OK) {
		...
		pr_err("BCMDHD_DIAG: attach: dscp_attach FAILED err=%d\n", err);
		goto cfg80211_attach_out;
	}
#endif /* defined(DHD_DSCP_POLICY) */
...
cfg80211_attach_out:
	pr_err("BCMDHD_DIAG: attach: reached cfg80211_attach_out, err=%d\n", err);
...
#define CONFIG_SAR_CONFIG_INFO_PATH "brcm/sarconfig.info"
#else
