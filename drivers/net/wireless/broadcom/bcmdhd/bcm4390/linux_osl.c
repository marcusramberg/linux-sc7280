...
{
	/* NOTE(mainline): strlcpy removed; use strscpy and return the destination
	 * to preserve the strncpy-like contract (callers expect a char *).
	 */
	strscpy(d, s, n);
	return d;
}
...
osl_timer_del(osl_t *osh, osl_timer_t *t)
{
	...
	if (t->set) {
		t->set = FALSE;
		/* NOTE(mainline): del_timer() was removed; call timer_delete() directly
		 * on the embedded struct timer_list (linuxver.h's del_timer macro wraps
		 * the now-removed kernel del_timer()).
		 */
		if (!timer_delete(&t->timer.timer)) {
#ifdef BCMDBG
