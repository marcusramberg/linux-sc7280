...
/* NOTE(mainline): net/lib80211.h removed upstream (lib80211 WEP/TKIP helper
 * library deleted); bcmdhd never referenced any lib80211 symbol, so the include
 * is simply dropped. linux/ieee80211.h (its only useful transitive include) is
 * still pulled in below. */
...
/* NOTE(mainline): del_timer()/del_timer_sync() were removed; map the compat
 * wrappers onto timer_delete()/timer_delete_sync(). */
