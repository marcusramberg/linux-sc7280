	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (((event == WLC_E_ASSOC_IND) || (event == WLC_E_REASSOC_IND)) &&
		...
		/* NOTE(mainline): cfg80211_new_sta() now takes a struct wireless_dev *. */
		...
		WL_INFORM_MEM(("[%s] del sta event for "MACDBG "\n",
			ndev->name, MAC2STRDBG(e->addr.octet)));
		/* NOTE(mainline): cfg80211_del_sta() now takes a struct wireless_dev *. */
	...
	struct wiphy *wiphy, uint8 link_id)
{
	...
	/* NOTE(mainline): cfg80211_ch_switch_notify() dropped the trailing
	 * puncturing-bitmap argument; it now takes (dev, chandef, link_id).
	 */
