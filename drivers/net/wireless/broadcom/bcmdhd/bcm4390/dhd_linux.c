...
fail:
	/* NOTE(mainline): unconditional diag -- DHD_ERROR is gated by dhd_msg_level,
	 * so the attach failure is invisible on the console; dhd_state names the
	 * last DHD_ATTACH_STATE_* milestone reached. Remove once attach is fixed. */
	pr_err("BCMDHD_DIAG: dhd_attach failed, dhd_state=0x%08x\n", dhd_state);
...
#endif /* DHD_MAP_LOGGING (mainline: handler is gated; komodo S2MPU is a no-op) */
