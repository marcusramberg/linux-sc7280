...
dhd_rx_pktpool_deinit(dhd_info_t *dhd)
{
	...
	/* NOTE(mainline): if dhd_rx_pktpool_init() never ran (dhd_attach failed
	 * earlier and unwinds through dhd_detach), rx_pool is zero-initialised.
	 * mainline skb_queue_purge_reason() treats a NULL skb_q.next as
	 * non-empty and dereferences it -> NULL-ptr oops. max_size is set only
	 * by dhd_rx_pktpool_init(), so use it to detect the un-inited pool. */
	if (!rx_pool->max_size)
		return;
