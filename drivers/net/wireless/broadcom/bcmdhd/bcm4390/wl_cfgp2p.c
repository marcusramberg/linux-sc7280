            ...
            s8 *oui, s32 ie_id, const s8 *data, s32 datalen, const s8* add_del_cmd)
{
	...
	/* NOTE(mainline): guard against an IE payload length that underflowed.
	 * Callers pass datalen = parsed_ie_len - VNDR_IE_FIXED_LEN, computed in
	 * unsigned arithmetic. An (extended) element shorter than the vendor-IE
	 * fixed header - e.g. an ID-255 element of length 2, which the parser
	 * admits because MIN_VENDOR_EXTN_IE_LEN(2) < VNDR_IE_MIN_LEN+2 - wraps to
	 * a huge value, arriving here as a negative s32 and driving a wild
	 * memcpy() (kernel panic). Such an IE cannot be represented on this
	 * OUI-based path, so skip it (add 0 bytes). Seen on WPA3/SAE connects. */
	if (datalen < 0) {
		CFGP2P_ERR(("wl_cfgp2p_vndr_ie: negative datalen %d, IE too short - "
			"skipping\n", datalen));
		return 0;
	}
