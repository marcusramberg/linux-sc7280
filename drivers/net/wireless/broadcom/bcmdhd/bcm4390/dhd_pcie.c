...
BCMFASTPATH(dhd_bus_dpc)(struct dhd_bus *bus)
{
	...
	if (!resched) {
		...
		if (!dhd_query_bus_erros(bus->dhd)) {
			if (bus->d2h_intr_control == PCIE_HOST_IRQ_CTRL) {
				/* NOTE(mainline): only re-enable the host IRQ if the
				 * ISR actually disabled it. The DPC can be scheduled
				 * without a preceding ISR (watchdog tick, resched),
				 * and enable_irq() on an already-enabled line triggers
				 * "Unbalanced enable for IRQ" WARNs whose in-DPC console
				 * stack dumps stall the datapath (-> link deauth).
				 */
				if (bus->host_irq_disable_count > bus->host_irq_enable_count) {
...
#ifdef DHD_PCIE_RUNTIMEPM
	bus->idlecount = 0;
	/* NOTE(mainline): keep the endpoint out of D3cold. dhd's D3 idle-suspend
	 * puts the EP in D3hot; the PCI core would otherwise promote D3hot->D3cold
	 * (parent drops slot power), and on this mainline pci-exynos setup that
	 * removed power and killed the link ("D3cold to D0, device inaccessible"
	 * -> deauth 34821). Pinned at D3hot the BCM4390 stays powered (firmware
	 * intact) and its PM-L1 exits when dhd restores D0, so pci_restore_state
	 * finds a live endpoint. This is what makes idle runtime-PM safe below. */
	if (bus->dev)
		pci_d3cold_disable(bus->dev);
	/* Idle runtime-PM is DISABLED (idletime=0). TESTED 2026-07-28 with
	 * MAX_IDLE_COUNT + pci_d3cold_disable + passive link_up: after ~31s idle
	 * the runtime-PM suspend goes through the WBRC as a full WLAN power-down
	 * and the re-on thrashes (wl2wbrc on/off loop) -> device wedges. The D3
	 * suspend/resume path is not viable on this mainline WBRC setup without a
	 * proper WBRC-aware relink; keep idle-PM off. Pinned at D0 the datapath is
	 * rock solid (2.9GB @ 103MiB/s clean). Do NOT re-enable without fixing the
	 * WBRC runtime power flow. */
	bus->idletime = 0;
