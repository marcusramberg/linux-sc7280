...
#if !IS_ENABLED(CONFIG_PCI_EXYNOS_GS)
/*
 * Default ON (full downstream BCM4390 L1SS/LTR + ASPM L1.2, per user directive).
 * Plain `modprobe bcmdhd4390` arms it; `support_l1ss=0` is the recovery escape
 * hatch if a build regresses the L1.2-exit path and wedges the link.
 */
uint32 support_l1ss = TRUE;
...
#if !IS_ENABLED(CONFIG_PCI_EXYNOS_GS)
	/*
	 * mainline pci-exynos (komodo): full downstream L1SS/LTR + ASPM L1.2 is
	 * driven via the `support_l1ss` module param (declared default TRUE = ON).
	 * Do NOT unconditionally force it TRUE here -- that clobbered the param, so
	 * `support_l1ss=0` (the recovery escape hatch, e.g. if an L1.2-exit
	 * regression wedges the link) could never take effect.
	 */
#endif /* !IS_ENABLED(CONFIG_PCI_EXYNOS_GS) */
...
{
#if defined(CONFIG_SOC_GOOGLE) || IS_ENABLED(CONFIG_PCI_EXYNOS)
	/*
	 * mainline (CONFIG_PCI_EXYNOS): the downstream path is CONFIG_SOC_GOOGLE-
	 * only, which left this a no-op -- so the faithful L1SS enable
	 * (_pcie_rc_l1ss_ctrl -> dhd_pcie_l1ss_ctrl -> google_pcie_wifi_l1ss ->
	 * zumapro_pcie_wifi_l1ss_enable) was compiled in but NEVER called, and
	 * support_l1ss did nothing.  Fire it on mainline too.  PCIE_L1SS_CTRL_WIFI
	 * is a discarded macro arg on the mainline _pcie_rc_l1ss_ctrl (line ~305),
	 * so it need not be defined here.
	 */
	DHD_CONS_ONLY(("%s: Control L1ss RC side %d\n", __func__, ctrl));
	_pcie_rc_l1ss_ctrl(ctrl, PCIE_L1SS_CTRL_WIFI, pcie_ch_num);
#endif /* CONFIG_SOC_GOOGLE || CONFIG_PCI_EXYNOS */
	return;
}
...
{
	int en = (support_l1ss && enable) ? 1 : 0;

	/* NOTE(mainline): drive the FULL downstream Wi-Fi L1SS+LTR+L1.2 sequence in
	 * pci-exynos (google_pcie_wifi_l1ss -> zumapro_pcie_wifi_l1ss_enable), a
	 * faithful port of exynos_pcie_rc_set_l1ss()'s EP_BCM_WIFI branch. The plain
	 * pci_enable_link_state() used here before was a no-op: pci-exynos trains
	 * the link with ASPM-L1 cleared, so the PCI core never enabled L1. The
	 * ported sequence re-advertises L1 in LNKCAP and programs the WiFi LTR
	 * thresholds / TCommon / TPowerOn the BCM4390 firmware needs -- without LTR
	 * active the firmware wedges its TX path under sustained load and the AP
	 * deauths the silent client. */
	DHD_PRINT(("%s: %s WIFI L1SS on ch%d (support_l1ss=%d)\n",
		__func__, en ? "enable" : "disable", ch_num, support_l1ss));
	return google_pcie_wifi_l1ss(ch_num, en);
}
...
{
	/* Real transient SW L1-exit (pci-exynos zumapro_pcie_wifi_l1_exit).
	 * This previously faked "L1 exit" as dhd_pcie_l1ss_ctrl(0, ...) -- a
	 * full L1SS TEARDOWN via config-space writes: wrong semantic (it
	 * permanently disarms L1.2 instead of transiently waking the link) and
	 * itself unsafe against a parked link.  Downstream-parity note: nothing
	 * calls dhd_plat_l1_exit on komodo downstream either; this is a
	 * recovery/diagnostic hook. */
	return google_pcie_wifi_l1_exit(ch_num);
}
#endif /* !IS_ENABLED(CONFIG_PCI_EXYNOS_GS) */
