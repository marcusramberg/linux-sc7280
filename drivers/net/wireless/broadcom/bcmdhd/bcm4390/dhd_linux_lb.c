...
dhd_napi_schedule(void *info)
{
	...
	if (napi_schedule_prep(&dhd->rx_napi_struct)) {
		...
		/*
		 * Downstream nudges ksoftirqd here via raise_softirq(NET_RX_SOFTIRQ)
		 * so the just-queued RX NAPI poll is serviced promptly (__napi_schedule
		 * only sets the softirq bit via __raise_softirq_irqoff; it does NOT wake
		 * ksoftirqd).  raise_softirq()/wakeup_softirqd() are not module-exported
		 * on mainline 7.1, so replicate the intent module-safely: in process
		 * context, run the pending NET_RX_SOFTIRQ through a self-balancing
		 * local_bh_enable() window (drains RX now, preventing a host-side RX
		 * backlog that can back-pressure the dongle's RX ring).  The pair nests
		 * to a no-op when BH is already disabled, and we skip it in hard/soft
		 * IRQ context (where the softirq is served on the natural exit anyway).
		 */
		if (!in_interrupt()) {
			local_bh_disable();
			local_bh_enable();
